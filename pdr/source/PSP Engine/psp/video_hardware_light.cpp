...
{/*
   ...
   // Allocate the vertices.
	...
	out[index].color = GU_RGBA(int(0.2f * 255.0f),
	                           int(0.1f * 255.0f),
						       int(0.0f * 255.0f),
						       int(1.0f * 255.0f));
	out[index].x = light->origin[0] - vpn[0]*rad;
    out[index].y = light->origin[1] - vpn[1]*rad;
    out[index].z = light->origin[2] - vpn[2]*rad;
	index++;
	
	for (i=16 ; i>=0 ; i--)
	{
		a = i/16.0 * M_PI*2;
		out[index].color = GU_RGBA(int(0.0f * 255.0f),
	                               int(0.0f * 255.0f),
						           int(0.0f * 255.0f),
						           int(1.0f * 255.0f));
	    out[index].x = light->origin[0] + vright[0]*cosf(a)*rad + vup[0]*sinf(a)*rad;
        out[index].y = light->origin[1] + vright[1]*cosf(a)*rad + vup[1]*sinf(a)*rad;
        out[index].z = light->origin[2] + vright[2]*cosf(a)*rad + vup[2]*sinf(a)*rad;
		index++;
	}
    sceGuDrawArray(GU_TRIANGLE_FAN, GU_VERTEX_32BITF | GU_COLOR_8888, 18, 0, out);
*/
}
